...
function Plugin:OnEnable()
    M:RegisterEvent("UPDATE_INVENTORY_DURABILITY", function() Plugin:Refresh() end)
    M:RegisterEvent("UPDATE_INVENTORY_ALERTS", function() Plugin:Refresh() end)
    M:RegisterEvent("PLAYER_REGEN_ENABLED", function() Plugin:Refresh() end)
end
...
function Plugin:Refresh()
    ...
    elseif hasItems then
        ...
        if lowest < 20 then color = "|cffff0000"
        elseif lowest < 50 then color = "|cffffff00" end
        display = string.format("%s%d%%|r", color, math.floor(lowest))
    else
        display = "|cffffffffN/A|r"
    end

    self.frame.text:SetText(string.format("|cff%sDurability:|r %s", classHex, display))
...
local function ShowTooltip(self)
    ...
    -- Class‑colored header: "PLAYER_NAME's Durability"
    local header = M:GetColoredPlayerHeader("Durability")
    GameTooltip:AddLine(header)
    GameTooltip:AddDoubleLine("Average Item Level:", string.format("|cffffff00%.1f|r", avgILvl), 1,1,1, 1,1,1)
    ...
    local totalCost = 0
    local cfg = M.db.plugins.durability
    ...
    for _, slotName in ipairs(displaySlots) do
        ...
        if link then
            ...
            if isHeirloom then
                right = string.format("|cff00ffff[%d]|r  |cff00ccffHeirloom|r", effective)
            elseif cur and max then
                local pct = math.floor((cur / max) * 100)
                local color = (pct < 20 and "|cffff0000") or (pct < 50 and "|cffffff00") or "|cff00ff00"
                right = string.format("|cff00ffff[%d]|r  %s%d%%|r", effective, color, pct)

                if cfg.showRepairCost then
                    scanner:ClearLines()
                    local hasItem, _, cost = scanner:SetInventoryItem("player", id)
                    if hasItem and cost then totalCost = totalCost + cost end
                end
            end
    ...
    if cfg.showRepairCost and totalCost > 0 then
        GameTooltip:AddLine(" ")
        GameTooltip:AddDoubleLine("Base Repair Cost (Neutral):", GetCoinTextureString(totalCost), 1,1,1, 1,1,1)
        GameTooltip:AddLine("-------------------------", 0.4,0.4,0.4)
...
-- Register plugin
--============================================================
M:RegisterPlugin("durability", Plugin)
